...
<head>
    <meta charset="UTF-8">
    <title>Menu Makanan - SederhanaOK</title>
...
<body class="bg-slate-900 text-slate-100 font-sans min-h-screen p-6 animate__animated animate__fadeIn">
    ...
    <div class="max-w-4xl mx-auto bg-slate-800 rounded-2xl p-6 shadow-2xl border border-slate-700">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-extrabold text-amber-400 flex items-center gap-2">🍔 Menu Makanan</h1>
            <span class="text-xs bg-amber-500/20 text-amber-300 px-3 py-1 rounded-full border border-amber-500/30">makanan.sederhanaok.com</span>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-700">
                    ...
                    <tr class="hover:bg-slate-700/30 transition-all duration-200">
                        ...
                        <td class="p-4 text-amber-400 font-semibold">Rp <?= number_format($row['harga']) ?></td>
                        <td class="p-4 text-center">
                            <form method="POST" class="flex justify-center items-center gap-2">
                                <input type="hidden" name="menu_id" value="<?= $row['id'] ?>">
                                <input type="number" name="jumlah" value="1" min="1" class="w-16 bg-slate-900 border border-slate-600 rounded-lg px-2 py-1 text-center text-white focus:outline-none focus:border-amber-400 transition">
                                <button type="submit" name="add_to_cart" class="bg-amber-500 hover:bg-amber-600 active:scale-95 text-slate-950 font-bold px-4 py-1.5 rounded-lg shadow-md transition-all duration-200 flex items-center gap-1">
                                    + Keranjang
                                </button>
                            </form>
                        </td>
        ...
        <div class="flex gap-4 mt-6">
            ...
            <a href="http://minuman.sederhanaok.com/minuman.php" class="bg-sky-600 hover:bg-sky-500 active:scale-95 px-5 py-2.5 rounded-xl font-medium transition duration-200">🥤 Ke Menu Minuman</a>
        </div>
    </div>

    <!-- Tombol Floating Keranjang -->
    <button onclick="toggleModal()" class="fixed bottom-6 right-6 bg-amber-500 hover:bg-amber-400 active:scale-90 text-slate-950 font-bold px-6 py-3.5 rounded-full shadow-2xl flex items-center gap-3 transition-all duration-300 animate__animated animate__pulse animate__infinite">
        <span class="text-xl">🛒</span>
        <span>Keranjang Saya</span>
        <span class="bg-slate-950 text-amber-400 text-xs px-2.5 py-1 rounded-full"><?= $total_items ?></span>
    </button>
    ...
    <div id="cartModal" class="hidden fixed inset-0 bg-black/70 backdrop-blur-sm flex justify-center items-center p-4 z-50 animate__animated animate__fadeIn">
        <div class="bg-slate-800 border border-slate-700 w-full max-w-lg rounded-2xl p-6 shadow-2xl relative animate__animated animate__zoomIn">
            ...
            <h2 class="text-2xl font-bold text-amber-400 mb-4 border-b border-slate-700 pb-2">🛒 Keranjang Belanja</h2>
            ...
            <div class="max-h-60 overflow-y-auto mb-4 border border-slate-700 rounded-xl p-2">
                ...
                ?>
                    <div class="flex justify-between items-center p-2 border-b border-slate-700/50">
                        ...
                        <span class="font-bold text-amber-400">Rp <?= number_format($sub) ?></span>
                    </div>
                <?php endforeach; else: ?>
            ...
            <?php if (!empty($_SESSION['cart'])): ?>
                <form method="POST">
                    ...
                    <input type="text" name="nama_pelanggan" required placeholder="Masukkan Nama Anda" class="w-full bg-slate-900 border border-slate-600 rounded-xl px-4 py-2.5 mb-4 text-white focus:outline-none focus:border-amber-400 transition">
